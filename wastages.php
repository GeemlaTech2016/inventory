...
if (isset($_POST['ok']) != '') {
    if ($_POST['id'] != '') {
        if (mysqli_query($conn, "update wastages set prodid='{$_POST["prodid"]}',qty='{$_POST["qty"]}',unit='{$_POST["unit"]}',dot='{$_POST["dot"]}',comment='{$_POST["comment"]}',culprit='{$_POST["culprit"]}',wtype='{$_POST["wtype"]}' where sn='{$_POST["id"]}'")) {
            $msg = '<div class="alert alert-success alert-dismissable">
                                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                                        Wastage Information Updated.
            </div>';
        } else {
        ...
        $getn = mysqli_num_rows(mysqli_query($conn, "select * from wastages"));
        $g = $getn + 1;
        $sn = "W" . $g . date('YmdHis');
        ...
        if (mysqli_query($conn, "insert into wastages(sn,prodid,qty,unit,comment,dot,culprit,wtype) values('$sn','{$_POST["prodid"]}','{$_POST["qty"]}','{$_POST["unit"]}','{$_POST["comment"]}','{$_POST["dot"]}','{$_POST["culprit"]}','{$_POST["wtype"]}')")) {
            mysqli_query($conn, "update products set totqty='$nqty',unit='{$_POST["unit"]}' where prodid='{$_POST["prodid"]}'");
            //mysqli_query("insert into transhistory(transid,dot,staffid,transtype,tco,receipt) values('$transid','{$_POST["asat"]}','$user','Purchase','$tco1','{$_POST["receipt"]}')");
            $msg = '<div class="alert alert-success alert-dismissable">
                                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                                        New Wastage Recorded.
                            		</div>';
        } else {
            $msg = '<div class="alert alert-danger alert-dismissable">
                                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
...
if (mysqli_query($conn, "delete from wastages where sn='$del'")) {
                                        ...
                                        <div class="banner-bottom-left">
                                            ...
                                            <div class="banner-bottom-left-grids">
                                                <div class="col-md-4 banner-left-grid fullwidth-left-grid login-right wow fadeInLeft" data-wow-delay="0.4s">
                                                    <h3>Add New Wastage</h3><p><?php echo $msg ?></p>
...
?>
                                                        ...
                                                        <div>
                                                            ...
                                                            <select name="wtype">
                                                                <option value="Leakage" selected>Leakage</option>
                                                                <option value="Disposed">Disposed</option>
                                                                <option value="Damaged">Damaged</option>
                                                            </select> 
                                                        </div>
                                                        <div>
                                                            <span>Addtional Comment</span>
                                                            <input type="text" name="comment" id="comment" value="<?php echo $gettypes['comment'] ?>"> 
                                                        </div>
                                                        <div>
                                                            <span>Date</span>
                                                        ...
                                                        <div>
                                                            ...
                                                            <select name="culprit">
                                                                ...
                                                                <?php
                                                                $q = mysqli_query($conn, "select * from employees");
                                                                while ($gb = mysqli_fetch_array($q)) {
                                                                    ?>
                                                                    <option value="<?php echo $gb['empid'] ?>"><?php echo $gb['empname'] ?></option>
    <?php
}
?>
                                                            </select> 
                                                ...
                                                <div class="col-md-8 banner-left-grid fullwidth-left-grid">
                                                    ...
                                                    <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                                        ...
                                                        <tbody>
                                                            ...
                                                            $b = 0;
                                                            $empname = "NA";
                                                            $q = mysqli_query($conn, "select * from wastages order by dot desc");
                                                            while ($rs = mysqli_fetch_array($q)) {
                                                                $b++;
                                                                $gp = mysqli_fetch_array(mysqli_query($conn, "select prodname from products where prodid='$rs[prodid]'"));
                                                                $gv = mysqli_fetch_array(mysqli_query($conn, "select empname from employees where empid='$rs[culprit]'"));
                                                                if ($gv == NULL) {
                                                                    $empname = "NA";
                                                                } else {
                                                                    $empname = $gv['empname'];
                                                                ...
                                                                <tr class="" style="font-size:10px;">
                                                                    ...
                                                                    <td><?php echo $rs['wtype'] ?></td>
                                                                    <td><?php echo $empname ?></td>
                                                                    <td><a href="addstock.php?id=<?php echo $rs['sn'] ?>">Edit</a>&nbsp;&nbsp;
                                                                        <a href="addstock.php?del=<?php echo $rs['sn'] ?>">Delete</a></td>
                                                                </tr>
    <?php
}
?>        
