...
if (isset($_GET['id']) && $_GET['id'] != '') {
    $id = $_GET['id'];
    $gettypes = mysqli_fetch_array(mysqli_query($conn, "select * from branches where branchid='{$_GET["id"]}'"));
}

if (isset($_POST['ok']) != '') {
    if ($_POST['id'] != '') {
        if (mysqli_query($conn, "update branches set brname='{$_POST["name"]}',address='{$_POST["address"]}' where branchid='{$_POST["id"]}'")) {
        ...
        $getn = mysqli_num_rows(mysqli_query($conn, "select * from branches"));
        $g = $getn + 1;
        if (mysqli_query($conn, "insert into branches(branchid,brname,address) values('$g','{$_POST["name"]}','{$_POST["address"]}')")) {
            $msg = '<div class="alert alert-success alert-dismissable">
                                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                                        ...
                                        <div class="banner-bottom-left">
                                            ...
                                            <div class="banner-bottom-left-grids">
                                                <div class="col-md-4 banner-left-grid fullwidth-left-grid login-right wow fadeInLeft" data-wow-delay="0.4s">
                                                    <h3>Add New Branch</h3><p><?php echo $msg ?></p>
                                                    <form name="form1" method="post" action="">
                                                        <div>
                                                            <span>Branch Name<label>*</label></span>
                                                            <input type="text" name="name" required value="<?php echo $gettypes['brname'] ?>"> 
                                                        </div>

                                                        <div>
                                                            <span>Address</span>
                                                            <input type="text" name="address" value="<?php echo $gettypes['address'] ?>"> 
                                                            <input type="hidden" name="id" value="<?php echo $gettypes['branchid'] ?>"> 
                                                        </div>
                                                ...
                                                <div class="col-md-8 banner-left-grid fullwidth-left-grid">
                                                    ...
                                                    <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                                        ...
                                                        <tbody>
                                                            ...
                                                            <?php
                                                            $b = 0;
                                                            $q = mysqli_query($conn, "select * from branches");
                                                            while ($rs = mysqli_fetch_array($q)) {
                                                                ...
                                                                <tr class="" style="font-size:10px;">
                                                                    ...
                                                                    <td><?php echo $rs['brname'] ?></td>

                                                                    <td><?php echo $rs['address'] ?></td>
                                                                    <td><a href="branches.php?id=<?php echo $rs['branchid'] ?>">Edit</a>&nbsp;&nbsp;
                                                                        <a href="branches.php?del=<?php echo $rs['branchid'] ?>">Delete</a></td>
                                                                </tr>
                                                                <?php
                                                            }
                                                            ?>        
