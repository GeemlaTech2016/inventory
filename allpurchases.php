		...
		<table width="100%" class="table table-striped table-bordered table-hover">
                               <thead>
                                    <tr style="font-size:14px">
                                        ...
                                        <th>Start Date</th>
                                        <th><input type="text" name="sdate" id="sdate" ></th>
                                        <th>End Date</th>
                                        <th><input type="text" name="enddate" id="enddate" ></th>
					...
					$b=0;
					$q = mysqli_query($conn,"select * from stock where dot>='$_POST[sdate] 00:00:00' and dot<='$_POST[enddate] 23:59:59' order by dot asc");
								?>
												<table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example1">
                               <thead>
                               <tr style="font-size:13px">
                                        <th colspan="6" style="text-align:center">Purchase Report from <?php echo $_POST['sdate']?> to <?php echo $_POST['enddate']?></th>
                                    </tr>
                                    <tr style="font-size:13px">
                                        <th>Date</th>
                                        <th>Product</th>
					...
					while($rs = mysqli_fetch_array($q)){
						$b++;
						$gp1 = mysqli_fetch_array(mysqli_query($conn,"select prodname from products where prodid='$rs[prodid]'"));
						$gv = mysqli_fetch_array(mysqli_query($conn,"select vendname from vendors where vendid='$rs[vendor]'"));
						$gr = mysqli_fetch_array(mysqli_query($conn,"select receipt from transhistory where transid='$rs[transid]'"));
						//$up = $up + $rs['unitprice'];
						$tp = $tp + $rs['tco'];
					?>            
        <tr class="" style="font-size:13px;">
                                      <td><?php echo $rs['dot']?></td>
                                        <td><?php echo $gp1['prodname']?></td>
                                        <td><?php echo number_format($rs['tco'],2)?></td>
                                        <td><?php echo $rs['totqty']." (".$rs['unit'].")"?></td>
                                        <td><?php echo $gv['vendname']?></td>
                                       <td><?php echo $gr['receipt']?></td>
        </tr>
                    <?php
					}
					if($b>0){
