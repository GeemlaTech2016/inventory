...
if(isset($_GET['id']) && $_GET['id']!=''){
	$id=$_GET['id'];
	$gettypes=mysqli_fetch_array(mysqli_query($conn,"select * from login where userid='{$_GET["id"]}'"));	
}

if(isset($_POST['ok'])!=''){
	if($_POST['id']!=''){
		if(mysqli_query($conn,"update login set password='{$_POST["password"]}',role1='{$_POST["rank"]}',status1='{$_POST["status1"]}' where userid='{$_POST["id"]}'")){
			...
			<div class="banner-bg"> 
					...
					<div class="banner">
						...
						<?php include("nav.php");?>
								...
								<h2><a href="">SEND SMS</a> <a href="smsunit.php">[Check SMS Balance]</a></h2>	
									...
									<div class="banner-bottom-left">
										...
										<div class="banner-bottom-left-grids">
                                            <div class="col-md-6 banner-left-grid fullwidth-left-grid login-right wow fadeInLeft" data-wow-delay="0.4s">
												<h3>Send Message to Users</h3><p><?php echo $msg?> </p>
												<form name="form1" method="post" action="sms.php">
												<div>
													<span>Sender ID<label>*</label></span>
													<input name="senderid" type="text" value="CLAIRENERGY" maxlength="11" required> 
												</div>
                                                <div>
													<span>RECIPIENTS<label>*</label></span>
													<textarea name="recipients" cols="75" rows="5" required placeholder="Enter Numbers seperated by comma"></textarea> 													  
												</div>
                                              
                                                <div>
													<span>MESSAGE<label>*</label></span>
													<textarea name="message" cols="75" rows="5" required placeholder="Type your Message here"></textarea> 													  
												</div>
                    ...
                    <?php
					$b=0; 
					//$q = mysqli_query("select * from login where role1='ceo' or role1='admin' and status1 = 'active'");
