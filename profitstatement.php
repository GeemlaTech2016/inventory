...
$gettypes=mysqli_fetch_array(mysqli_query($conn,"select * from kgpricetb where prodid='PA1'"));
?>
<!DOCTYPE html>
<html>
<head>
...
<div class="container">
	<div class="bg-banner">
			...
			<div class="banner-bg"> 
					...
					<div class="banner">
						...
						<?php include("nav.php");?>
								...
								<h2><a href="">PROFIT STATEMENT</a></h2>	
		...
		<table width="100%" class="table table-striped table-bordered table-hover">
					...
					$b=0; 
					$cp= 0.0;$sp= 0.0;$pn= 0.0;$pb= 0.0;
					$n= 0.0; $p= 0.0;
					$q = mysqli_query($conn,"select * from sales where dot>='$_POST[sdate] 00:00:00' and dot<='$_POST[enddate] 23:59:59' order by dot asc");
								?>
												<table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example1">
                               <thead>
                               <tr style="font-size:13px">
                                        <th colspan="7" style="text-align:center;">Profit Statement from <?php echo $_POST['sdate']?> to <?php echo $_POST['enddate']?></th>
                                    </tr>
                                    <tr style="font-size:13px">
                                        <th>Date</th>
                                        <th>Product</th>
					...
					while($rs = mysqli_fetch_array($q)){
						$b++;
						$gpp = mysqli_fetch_array(mysqli_query($conn,"select * from `productcat` where `catid`='{$rs['prodid']}'"));
		
		$n = ($gettypes['sprice'] * $gpp['cylindersize']) - ($gettypes['cprice'] * $gpp['cylindersize']);
		
		$cp = $cp + ($gettypes['cprice'] * $gpp['cylindersize']);
		$sp = $sp + ($gettypes['sprice'] * $gpp['cylindersize']);
		$p =  $p + $n;
					?>            
        <tr class="" style="font-size:12px;">
                                      <td><?php echo $rs['dot']?></td>
                                        <td><?php echo $gpp['category']?></td>
                                        <td><?php echo $rs['qty']?></td>
                                        <td><?php echo number_format($gettypes['cprice']*$gpp['cylindersize'],2)?></td>
                                        <td><?php echo number_format($gettypes['sprice']*$gpp['cylindersize'],2)?></td>
                                        <td><?php echo number_format($n,2)?></td>
                                       <td>&nbsp;</td>
        </tr>
                    <?php
					}
